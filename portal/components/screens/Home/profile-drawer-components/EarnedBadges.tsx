    ...
    <div className="pb-3">
      ...
      <div className="">
        {loading ? (
          ...
          <div className="custom-scrollbar flex flex-row items-center gap-4 overflow-x-scroll">
            {badgeRewarded?.map((badge: BadgeRewarded) => (
              <div
                className="flex w-24 flex-col items-center justify-center gap-1"
                key={badge.id}
              >
                <div className="hover:before:animate-shine focus:before:animate-shine relative h-full w-24 overflow-hidden rounded-full before:absolute before:left-[-75%] before:top-0 before:z-[2] before:h-full before:w-1/4 before:skew-x-[-25deg] before:transform before:bg-[linear-gradient(to_right,rgba(255,255,255,0.4)_0%,rgba(255,255,255,0.4)_100%)] before:content-['']">
                  <img
                    key={badge.id}
                    src={badge.imageUrl || ''}
                    alt={badge.name}
                    className="h-24 w-24 rounded-full border-4 border-gray-300 object-cover shadow-lg"
                  />
                </div>
                <p className="px-1 text-center text-xs font-semibold capitalize">
                  {badge.name}
                </p>
              </div>
            ))}
