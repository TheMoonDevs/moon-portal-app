    ...
    <div className="calendar">
      <div className="calendar-body">
        <div className="grid grid-cols-7 gap-1 bg-neutral-100">
          {days.map((day) => (
            <div key={day} className="p-3 font-mono text-sm tracking-widest">
              <span>{day}</span>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-2">
          {Array.from({ length: firstDayOfThisMonth }).map((_, index) => {
            ...
            return (
              <div
                key={index}
                className="text-uppercase p-3 text-sm tracking-widest text-neutral-200"
              >
          ...
          {Array.from({ length: currentMonthDayjs.daysInMonth() }).map(
            (_, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`text-uppercase relative flex cursor-pointer items-center justify-center p-3 text-sm tracking-widest ${
                    isToday ? 'rounded-full bg-neutral-100 font-black' : ''
                  }`}
                  onClick={() => {
                ...
                >
                          ...
                          trackerMode === 'leave'
                            ...
                            : trackerMode === 'zero'
                              ...
                              : trackerMode === 'meeting'
                                ? 'red'
                                : 'unknown'
                        }-500 absolute inset-y-0 size-full rounded-full`}
                      ></span>
                    )}
                  <span
                          ...
                          dateFromRecords?.type === 'zero'
                            ...
                            : dateFromRecords?.type === 'leave'
                              ...
                              : dateFromMeeting.type === 'meeting'
                                ? 'bg-red-500'
                                : 'bg-neutral-200'
                        } absolute right-[40%] top-1 size-2 rounded-full`}
                      ></span>
                    )}
                  ...
                  {dateFromMeeting && trackerMode === 'meeting' && (
                    <span
                      className={`absolute right-[40%] top-1 size-2 rounded-full bg-red-500`}
                    ></span>
                  )}

                  {meetingDate === date.format('YYYY-MM-DD') &&
                    trackerMode === 'meeting' && (
                      <span className="absolute right-[40%] top-1 size-2 rounded-full bg-red-500"></span>
                    )}
                </div>
              );
            },
