...
export interface UseWorksheetRowsResult {
  ...
  bulkUpdateRows: (
    worksheetId: string,
    updates: { id: string; rawPayload: Partial<Record<string, unknown>> }[],
  ) => Promise<void>;
  addRow: (
    worksheetId: string,
    rawPayload?: Record<string, unknown>,
  ) => Promise<WorksheetRowData | null>;
    ...
    async (
      ...
      try {
        ...
        const res = await fetch(
          `/api/worksheets/${encodeURIComponent(worksheetId)}/rows/${encodeURIComponent(id)}`,
          {
            method: 'PATCH',
            headers: {
  ...
  const bulkUpdateRows = useCallback(
    async (
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(await readApiError(res, 'Failed to bulk update'));
        const json = await res.json();
  ...
  const addRow = useCallback(
    async (
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(await readApiError(res, 'Failed to add row'));
        const json = await res.json();
