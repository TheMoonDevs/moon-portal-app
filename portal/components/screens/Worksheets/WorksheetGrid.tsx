...
interface WorksheetGridProps {
  ...
  rowHeight?: number;
  onSelectionChange?: (cellOrRange: { rowId?: string; field?: string; rowIds?: string[]; fields?: string[]; selectedCells?: { rowId: string; field: string }[] } | null) => void;
  onCellValueChange?: (rowId: string, rawPayload: Partial<Record<string, unknown>>) => void | Promise<void>;
  onGridReady?: () => void;
...
export function WorksheetGrid({
  ...
  const uiState = useWorksheetStore((state) => state.uiStateByWorksheet[worksheetConfig.id] || DEFAULT_UI_STATE);
    ...
    const baseColDef: ColDef = {
      ...
      cellClassRules: {
        'bg-blue-100/30 outline outline-1 outline-blue-500/50 z-10 relative': (params) => {
          ...
          if (sel.type === 'cell') return sel.rowId === rowId && sel.field === field;
          if (sel.type === 'range' && sel.selectedCells?.length) {
            return sel.selectedCells.some((c) => c.rowId === rowId && c.field === field);
          }
          return false;
        }
      },
      cellStyle: (params) => {
        ...
        const isSelected =
          sel.type === 'cell'
            ...
            : !!(
                sel.type === 'range' &&
                sel.selectedCells?.some((c) => c.rowId === rowId && c.field === field)
              );
        ...
        if (cStyle) {
          if (cStyle.backgroundColor) style.backgroundColor = cStyle.backgroundColor;
          ...
          if (cStyle.textDecoration) style.textDecoration = cStyle.textDecoration;
            ...
            {(col as import('@/lib/worksheets/core/types').ActionsColumnConfig).actions.map(
              (actionDef) => (
                <button
                  key={actionDef.id}
                  type="button"
                  className={`px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded-none border-2 border-black cursor-default
                    ${
      ...
      if (numCol.numberFormat) {
        baseColDef.valueFormatter = (p) => {
          ...
          if (numCol.numberFormat === 'currency') {
            const currency = numCol.currencyField && p.data
            ...
            return new Intl.NumberFormat('en-US', { style: 'percent', maximumFractionDigits: numCol.decimalPlaces ?? 2 }).format(num / 100);
          } else {
            return new Intl.NumberFormat('en-US', { maximumFractionDigits: numCol.decimalPlaces ?? 2 }).format(num);
          }
        };
      }
    } else if (col.type === 'date') {
        ...
        baseColDef.valueFormatter = (p) => {
          ...
          try {
            ...
            return new Intl.DateTimeFormat('en-US', {
              dateStyle: dateCol.dateFormat as 'short' | 'medium' | 'long' | 'full'
            }).format(date);
          } catch(e) {
            return p.value;
          }
  ...
  const columnDefs = useMemo<ColDef[]>(() => {
    ...
    return cols;
  }, [worksheetConfig, columnConfigToAgGrid, uiState.pinnedCols, uiState.hiddenCols]);
  ...
  useEffect(() => {
    ...
    const state = columnDefs
      ...
      .map((c) => {
        ...
        const pinned: 'left' | 'right' | null = pinnedCols.includes(id) ? 'left' : null;
  ...
  const handleCellClicked = useCallback((event: CellClickedEvent) => {
    ...
    let newSelectedCells = selection.selectedCells ? [...selection.selectedCells] : [];
    ...
    if (isShift && anchorCell) {
      ...
      for (let r = minRowIdx; r <= maxRowIdx; r++) {
        ...
        if (rowNode && rowNode.data) {
          for (let c = minColIdx; c <= maxColIdx; c++) {
            rangeCells.push({ rowId: String(rowNode.data.id), field: colIds[c] });
          }
        }
      }
      ...
      if (isCtrl) {
        ...
        rangeCells.forEach(rc => {
          if (!newSelectedCells.some(sc => sc.rowId === rc.rowId && sc.field === rc.field)) {
      ...
      if (selection.type === 'cell' && selection.rowId && selection.field) {
        if (!newSelectedCells.some(sc => sc.rowId === selection.rowId && sc.field === selection.field)) {
          newSelectedCells.push({ rowId: selection.rowId, field: selection.field });
        }
      }

      const existingIdx = newSelectedCells.findIndex(sc => sc.rowId === rowId && sc.field === field);
      if (existingIdx >= 0) {
  ...
  const handleCellValueChanged = useCallback((event: CellValueChangedEvent) => {
    if (event.data?.id && event.colDef.field && event.newValue !== event.oldValue) {
      onCellValueChange?.(String(event.data.id), { [event.colDef.field]: event.newValue });
  ...
  const gridKey =
    selection.type === 'cell' && selection.rowId && selection.field
      ...
      : selection.type === 'range' && selection.selectedCells?.length
        ? `sel-${selection.selectedCells.map((c) => `${c.rowId}-${c.field}`).sort().join('|')}`
        : 'sel-none';
