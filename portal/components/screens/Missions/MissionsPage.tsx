...
import { HOUSEID, Mission, USERROLE, USERSTATUS, USERTYPE, User } from "@prisma/client";
import dayjs from "dayjs";
import { Dispatch, SetStateAction, useEffect, useState } from "react";
import { ChevronDownIcon } from "lucide-react";


const MissionTasks = ({ tasks, mission, setMission, coreTeam, setSelectedMission }: {
    tasks: MissionTask[];
    mission: Mission;
        ...
        <div className="mission-tasks p-4 border border-neutral-400">
            <div className="flex items-center gap-4 pb-4">
                ...
                <button
                    ...
                    className="border border-black text-black px-4 py-2 text-xs">Add</button>
                <button
                    onClick={() => setMission(null)}
                    className="bg-black text-white px-4 py-2 text-xs">Close</button>
                <button
                    onClick={() => {
                        setMission(null);
                        setSelectedMission(mission, null)
                    }}
                    className="bg-green-500 text-white px-4 py-2 text-xs">Save</button>
            </div>
            ...
            {tasks?.map((task) => (
                <ul key={task.userId}>
                    <div className="grid grid-cols-10 items-center gap-2" key={task.userId}>
                        <input
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                setSelectedTask((sm) => {
                                    let sel_user = coreTeam.find((user) => user.id === e.target.value);
                                    let userInfo = {
                        ...
                        >
                            {coreTeam.map((sel_user) => (
                                <option key={sel_user.id} value={sel_user.id}>
                                    <img src={sel_user.avatar ?? ""} alt={sel_user.name ?? ""} className="w-6 h-6 rounded-full" />
                                    {sel_user.name}
                                </option>
                            ))}
                        </select>
...
export const MissionsPage = () => {
    ...
    useEffect(() => {
        PortalSdk.getData(
            "/api/user?role=" + USERROLE.CORETEAM + "&userType=" + USERTYPE.MEMBER + "&status=" + USERSTATUS.ACTIVE,
            null
        )
            .then((data) => {
            ...
            <div className="missions-page">
                <div className="flex flex-row w-full justify-between  p-4">
                    ...
                    <button
                        ...
                        className="bg-black text-white px-4 text-md">Create Mission</button>
                </div>
                ...
                {missions.map((mission) => (
                    <ul key={mission.id}>
                        <div
                            ...
                            className="grid grid-cols-11 items-center gap-2" key={mission.id}>
                            <button className="flex items-center border border-neutral-300 px-4 text-md"
                            ...
                            />
                            <p>{
                                mission.indiePoints - (mission.tasks as any[] || []).reduce((acc, task) => acc + task.indiePoints, 0)
                            }</p>
                            {/* <DatePicker
                            className="border border-neutral-300  px-4"
