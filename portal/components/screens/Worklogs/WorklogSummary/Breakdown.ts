...
const calculateProductivityRatio = (
...
const findProductiveDays = (
  ...
  Object.keys(dailyTasks).forEach((day) => {
    ...
    const productivityRatio = calculateProductivityRatio(
      completedTasks,
      totalTasks,
    );
  ...
  ArrayHelper.forwardSortByDate(worklogSummary, 'date').forEach((log) => {
    ...
    log.works.forEach((work: any) => {
      ...
      if (!weeksData[weekLabel].dailyTasks[day]) {
        weeksData[weekLabel].dailyTasks[day] = {
          totalTasks: 0,
          completedTasks: 0,
        };
      }
...
export const getWeekData = (groupedByWeek: any) => {
  return Object.keys(groupedByWeek).map((weekLabel) => {
    ...
    const { mostProductiveDay, leastProductiveDay } =
      findProductiveDays(dailyTasks);

    const mostProductiveDate = formatDay(mostProductiveDay);
    const leastProductiveDate = formatDay(leastProductiveDay);

    const completedTasksMost =
      dailyTasks[mostProductiveDay]?.completedTasks || 0;
    const totalTasksMost = dailyTasks[mostProductiveDay]?.totalTasks || 0;

    const completedTasksLeast =
      dailyTasks[leastProductiveDay]?.completedTasks || 0;
    ...
    return {
      ...
      mostProductiveDay: mostProductiveDay
        ? `${mostProductiveDate} (${completedTasksMost}/${totalTasksMost})`
        : 'N/A',
      leastProductiveDay: leastProductiveDay
        ? `${leastProductiveDate} (${completedTasksLeast}/${totalTasksLeast})`
        : 'N/A',
    };
  });
};
