...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const currentDir = await prisma.directoryList.findUnique({
    ...
    const siblingDir = await prisma.directoryList.findFirst({
      where: {
    ...
    return NextResponse.json({
      status: "success",
      data: {
        message: `Directory moved ${direction} successfully`,
        changed,
      },
    });
  } catch (e) {
