import type { BOTMODE, FUNCTIONTYPE } from '@db/client';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const clientFunction = await db.clientRequestFunction.create({
      data: {
        clientId,
        botProjectId,
        originClientRequestId,
  ...
  try {
    ...
    if (!id && !clientId && !botProjectId && !clientRequestId && !name) {
      return NextResponse.json(
        {
          error:
            'Missing id, name, clientId, botProjectId, or clientRequestId parameter',
        },
        { status: 400 },
      );
    }

    const clientFunctions = await db.clientRequestFunction.findMany({
      where: {
        ...(id && { id }),
        ...(name && { name }),
        ...(clientId && { clientId }),
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const clientFunction = await db.clientRequestFunction.findUnique({
      where: { id },
    });
    ...
    const updatedFunction = await db.clientRequestFunction.update({
      where: { id },
      data: updateData,
    });
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // Delete the function
    await db.clientRequestFunction.delete({
      where: { id },
    });
