...
export async function GET(request: NextRequest) {
  try {
    ...
    const existingConfig = await db.configData.findFirst({
      where: {
    ...
    const isDateDifferent = emailTracker.some(
      (log: { id: string }) => log.id !== currentDate,
    );
    ...
    if (mailId) {
      const emailLog = emailTracker?.find(
        (log: { mailId: string }) => log.mailId === mailId,
      );
