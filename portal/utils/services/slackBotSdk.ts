  ...
  sendSlackMessageviaAPI = async (message: SlackMessageType) => {
    try {
      ...
      if (message.user_id && !message.channel) {
        channel = message.user_id; // Use user_id as channel if channel is not provided
      } else if (message.channel && !message.user_id) {
        channel = message.channel; // Use channel if channel is provided and user_id is not
      } else if (message.channel && message.user_id) {
        channel = message.user_id; // Use user_id if both are provided
      } else {
