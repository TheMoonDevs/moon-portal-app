...
export const numberWithCommas = (x: number | string, decimals = 0) => {
  if (!x) return "0";
  let x_number = typeof x == "number" ? x : parseFloat(x);
  x_number = decimals > 0 ? parseFloat(x_number?.toFixed(decimals)) : x_number;
  var parts = x_number.toString().split(".");
...
const truncateRegex = /^([\w\d]{6})[\w\d]*([\w\d]{4})$/;
// Captures 0x + 8 characters, then the last 8 characters.
const truncateLongerRegex = /^([\w\d]{10})[\w\d]*([\w\d]{8})$/;

export const prettyEthAddress = (address: string) => {
  if (!address) return "";
  const match = address.match(truncateLongerRegex);
  if (!match) return address;
  return `${match[1]}…${match[2]}`;
};

// Function to prettily print a hash
export const prettyHash = (hash: string) => {
  if (!hash) return "";
...
export const prettyPrintISOStringInMMMDDYYYYHHMM = (dateString: string) => {
  const date = new Date(dateString);
  const month = date.toLocaleString("default", { month: "short" });
  const day = date.getDate();
  const year = date.getFullYear();
  const hours = date.getHours();
  const minutes = date.getMinutes().toString().padStart(2, '0'); // pad with zero if minutes is less than 10
